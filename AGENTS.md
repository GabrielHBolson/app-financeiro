...
- `(tabs)/` — 6 abas:
  - `index` Dashboard: seletor de mês, saldo do mês = recebidos − gastos − investidos, cards de estatísticas, total de contas ativas, lançamentos do mês
  - `expense` Gastos, `income` Recebidos, `investments` Investimentos — todos usam `TransactionManager`
  - `bills` Contas mensais: criar recorrentes ou com prazo (parcelas), ativar/pausar, marcar pago do mês, excluir
  - `profile` Perfil: editar nome, toggle de biometria, CRUD de categorias, sair da conta
- `bill/[id]` — detalhe de uma conta: parcelas (contas com prazo) ou histórico por mês (recorrentes), marcar/desfazer pagamento, anotação por pagamento
...
- **Contas mensais:** recorrente (todo mês, sem fim) ou com quantidade de meses + mês de início (parcelas); dia de vencimento 1–31; marcar/desfazer pagamento por mês; anotação por pagamento.
- **Notificações:** lembrete de vencimento às 09:00 do dia de vencimento (`syncBillReminders`), resincronizadas ao abrir o app e a cada mudança de contas/pagamentos.
- **Dashboard:** `balance = income − expense − invested`.
...
- Card azul "Saldo do mês" = recebidos − gastos − investidos; texto vermelho claro se negativo.
- 3 cards de estatísticas do mês: Recebido (verde, `arrow-up-circle`), Gasto (vermelho, `arrow-down-circle`), Investido (azul, `trending-up`).
- Card "Contas do mês": nº de contas ativas + soma dos valores (`formatCurrency`).
...
- Dados carregados em `useFocusEffect`: `getProfile`, `getTransactions(monthStartISO, monthEndISO)`, `getMonthlyBills`.

### `(tabs)/expense` Gastos · `(tabs)/income` Recebidos · `(tabs)/investments` Investimentos
- Telas finas que apenas renderizam `TransactionManager` com `type` ("expense" | "income" | "investment") e `title` ("Gasto" | "Recebido" | "Investimento").
- `TransactionManager`:
...
- Card azul "Total de contas ativas" (soma das contas com `active`).
...
- Lista "Suas contas" (FlatList):
  - Tap na linha → `bill/[id]`.
  - Nome, "Dia {n} · {valor} · {n} pagas"; parcelas mostram "x/y pagas" ou "Concluída · x/y" e badge "{total} meses".
  - Check (círculo) marca/desmarca pagamento do mês atual (`monthStartISO()`), desabilitado se conta inativa.
  - `Switch` ativa/pausa a conta (`toggleMonthlyBillActive`).
  - Lixeira exclui com `Alert` de confirmação.
  - Conta inativa ou concluída: opacidade 0.5, nome riscado.
...
- Conta com prazo (`is_recurring=false`): lista "Parcelas" gerada de `start_month`/`created_at` + `total_months` via `addMonths`; cada linha "Parcela {i}" com mês, valor e check.
- Conta recorrente: `MonthPicker` + linha do mês selecionado (mesmo componente `PaymentRow`).
- Linha paga: nome/mês riscado, valor verde, check preenchido; linha mostra `TextInput` de anotação ("paguei com PIX...").
...
- `bill_payments` — `bill_id`, `user_id`, `month`, `amount`, `description`, `paid_at`; unique `(bill_id, month)`
...
- Helpers de moeda/data no `utils/format.ts` (`formatCurrency`, `parseAmount`, `monthStartISO`, `monthEndISO`, `addMonths`, `currentMonthLabel`, `formatDate`, `toISODate`).
- Acesso a dados via `services/api.ts` (supabase client em `services/supabase.ts`).
